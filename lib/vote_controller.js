(function () {
    ...
    ********************************************************************************/
    ...
    voteController.updateVote = function(table, id, vote, callback){
        ...
        if( vote === UPVOTE ){
            statement += 'up = up + 1, ' +
                         'percentageUp = (up + 1) / (total + 1) ';
        } else {
            statement += 'down = down + 1, ' +
                         'percentageUp = up / (total + 1) ';
        }
...
// console.log( userId + ' ' + postId + ' ' + vote );
...
// console.log(userVotes);
    ...
    */
    ...
    var createGroups = function(numberOfGroups, callback){
        var groupObjs = Array.apply(null, Array(numberOfGroups)).map(function() { return {} });
        norm.table('Groups')
            .insert(groupObjs, function(err){
                if( err ){ callback(err); }
                else{ callback(); }
            });
    }
...
//////////// new seed functions
    ...
    var createSeedUsers = function(numberOfUsers, callback){
    ...
    var createSeedPosts = function(numberOfPosts, userId, callback){
        var posts = [];
        ...
        norm.table('Posts')
            .insert(posts, callback);
    }

    var getRecentIds = function(table, numberOfIds, callback){
        norm.table(table)
            .select(['id'])
            .limit(numberOfIds)
            .orderDesc('id')
            .findAll(function(err, users){
                if( err ){ callback(err); }
                else{
                    callback(
                        null, 
