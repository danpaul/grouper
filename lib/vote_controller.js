(function () {
    ...
    ********************************************************************************/
    ...
    // var insertUserVote = voteController.insertUserVote = function()

// voteController.upsertVote = function()
...
voteController.getMultiKeyVoteQuery = function(
    ...
    if( vote === UPVOTE ){
        statement += 'INSERT INTO ?? (??, ??, up, total, percentage_up) VALUES (?, ?, 1, 1, 1.0) ';
        statement += 'ON DUPLICATE KEY UPDATE total = total + 1, ';
        statement += 'up = up + 1, percentage_up = up / total ';
    } else {
        statement += 'INSERT INTO ?? (??, ??, down, total, percentage_up) VALUES (?, ?, 1, 1, 0.0) ';
        statement += 'ON DUPLICATE KEY UPDATE total = total + 1, ';
        statement += 'down = down + 1, percentage_up = up / total ';
    }

//     statement += 'SELECT * FROM ?? WHERE ??=? AND ??=?';
//     params.push(table, columnOne, valueOne, columnTwo, valueTwo);
// console.log(knex.raw(statement, params).toString());
    // return knex.raw(statement, params).toString() + '; ';
    ...
    voteController.updateVote = function(table, id, vote, callbackIn){
        ...
        knex.raw(statement, params)
            ...
            .catch(callbackIn)
        // norm.executeRaw(statement, params, callback);
    }
                        ...
                        .then(function(){
                            ...
                            knex('post_group_votes')
                                ...
                                .then(function(groupVote){
                                    ...
                                    //determine agreement
                                    var userAgreementVote = voteController.getUserAgreementVote(
                                                                                vote,
    ...
    //     .where('UserId', '=', userId)
    //     .select(['GroupId'])
    //     .findAll(function(err, groupRows){
    //         if(err){ callback(err); }
    //         else{
    //             var groupIds = [];
    //             groupRows.forEach(function(r){ groupIds.push(r.GroupId) });
    //             //use insert ignore here
    ...
    var castUserVote =  voteController.castUserVote = function(
        ...
        //         // should return no results, test if user has already voted
    ...
    ********************************************************************************/
    ...
    var seed = voteController.seed = function(callbackIn){
        ...
        var numberOfGroupsUserBelongsTo = 2;
        ...
        async.series([
            ...
            // function(callback){ createGroups(numberOfGroups, callback) },
            function(callback){
                getRecentIds('groups', numberOfGroups, function(err, groupIdsIn){
                    if( err ){ callback(err); }
                    else{ groupIds = groupIdsIn; callback(); }
                });
            },
            // function(callback){ createSeedUsers(numberOfUsers, callback); },
            function(callback){
                getRecentIds('users', numberOfUsers, function(err, userIdsIn){
                    if( err ){ callback(err); }
                    else{ userIds = userIdsIn; callback(); }
                });
            },
            // function(callback){ createSeedPosts(numberOfPosts, userIds[0], callback) },
            function(callback){
                getRecentIds('posts', numberOfPosts, function(err, postIdsIn){
                    if( err ){ callback(err); }
                    else{ postIds = postIdsIn; callback(); }
                });
            },
            // function(callback){
            //     assignUsersToGroups(groupIds, userIds, numberOfGroupsUserBelongsTo, callback)
            // },
            function(callback){
